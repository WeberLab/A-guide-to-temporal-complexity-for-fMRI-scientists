...
Complexity is possibly best conceptualized as involving a delicate balance between order and disorder [@spornsNetworksBrain2010]. Uniform systems (e.g., crystal lattices; sine waves) are not complex, nor are completely irregular ones (e.g., randomly moving particles; white noise); rather, complexity lies between these two extremes, combining predictability and unpredictability [@varleyConsciousnessBrainFunctional2020]. In this model, the brain is considered a dynamical system operating at a phase transition between these two opposing states. At this transition --- called the critical point --- information processing is optimized [@beggsNeuronalAvalanchesNeocortical2003; @marshallAnalysisPowerLaws2016; @carhart-harrisEntropicBrainTheory2014; @carhart-harrisEntropicBrainRevisited2018]. 

To understand how a system poised between order and disorder allows for optimal optimal information processing, it is important to understand that all critical systems display something known as scale-invariance [@hengenCriticalityUnifiedSetpoint2024]. When a system is scale-invariant, it is self-similar across all scales; that is, it has a "fractal" structure, containing meaningful information at every level of magnification. Scale-invariance further implies that the system has memory, with past events influencing the next milliseconds, seconds, and minutes [@hengenCriticalityUnifiedSetpoint2024]. By allowing past information to be used in the future, this "memory" supports optimal information processing [@hengenCriticalityUnifiedSetpoint2024]. 

<!-- TODO: make sure this section doesn't repeat too much from the Hurst section --> 
<!-- TODO: add more about criticality --> 

# Criticality and neuronal avalanches

What are avalanches? 
<!-- TODO: explain avalanches from a physics perspective --> 
With respect to the brain, avalanches are a special form of what are called "neural cascades" [@beggsCortexCriticalPoint2022]. Cascades are trails of activation spreading from neuron to neuron, where spontaneous activity in one neuron triggers activity in its neighbour. Some cascades are large, involving many activations over a long time window, while some are small, involving a few activations and dying off quickly. At the critical point, the distribution of cascade sizes and durations follows a power law distribution, where there are many small and short cascades and few large and long ones (@fig-avalanche and @fig-temporalpropavalanches). Additionally, there should also be a power-law relationship between size and duration. Cascades with these power-law properties are called avalanches. Note that neuronal avalanche analysis would not strictly qualify as temporal complexity, as computations of both size and duration take information from multiple voxels regions into account.

![A) In fMRI, the method used to detect avalanches was introduced by Tagliazucchi et al. (2012). The BOLD time series, either representing a single voxel or the mean of an ROI, is converted to a point process by first normalizing the series by its own SD, then setting a threshold equivalent to a multiple of the SD, and selecting the time points at which the series crosses the threshold from below. The same procedure is repeated for all voxels and ROIs. Panel based on @tagliazucchiCriticalityLargeScaleBrain2012. B) An avalanche is defined as a series of consecutively active time bins, surrounded by empty bins. The size of the avalanche is the total number of activations --- across all voxels or ROIs --- and the duration is the total number of time bins. Panel based on @xinGeneticContributionsBrain2025. C-E) If the system is near the critical point, the distribution of cluster sizes ($P(S)$), durations ($P(T)$), and average size for a given duration $<S>(T)$ can be fit to power laws with critical exponents $\alpha$, $\tau$, and $\gamma$ --- where $\gamma$ is related to $\alpha$ and $\tau$ via the relationship shown in panel E), and ideal exponents are $\alpha$ = 1.5 and $\tau$ = 2 (figure based on models in @tagliazucchiCriticalityLargeScaleBrain2012; also see @beggsNeuronalAvalanchesNeocortical2003; @shewFunctionalBenefitsCriticality2013; @petermannSpontaneousCorticalActivity2009; @yangNaturalBrainState2023). ](./images/300_dpi/Fig17.png){#fig-avalanche width=450}

![The temporal properties of avalanches can be further described by plotting their *temporal profiles* --- the activation in each frame across time. Across scales, these profiles display the shape of an inverted parabola. That is, the shape of the temporal profile is the same regardless of the duration of the avalanche, scaled up if the avalanche is long and down if it is short. Note that this "fractal" structure is what allows avalanches to be studied across modalities, from very fine temporal resolutions (e.g., local field potential recordings [@beggsNeuronalAvalanchesNeocortical2003] to coarse ones (e.g., fMRI [@tagliazucchiCriticalityLargeScaleBrain2012]).](./images/300_dpi/Fig18.png){#fig-temporalpropavalanches width=450}

Some of the earliest discussions of criticality in neuroscience emerged from avalanche data, and in contemporary work, research on the critical point continues to mainly focus on neuronal avalanches [@beggsNeuronalAvalanchesNeocortical2003; @marshallAnalysisPowerLaws2016]. Avalanches have been studied at multiple scales, from the level of single cells [@bellaySelectiveParticipationSingle2021], and local field potentials [@beggsNeuronalAvalanchesNeocortical2003], to whole-brain imaging techniques like EEG [@benayounEEGTemporalCorrelations2010], MEG [@shrikiNeuronalAvalanchesResting2013], and fMRI [@tagliazucchiCriticalityLargeScaleBrain2012] (see @tbl-avalanche). Because of scale-invariance, across all these modalities, avalanche properties can be fit to power laws with relatively consistent exponents [@hengenCriticalityUnifiedSetpoint2024].

| Reference | Sample size | Age (mean ± sd or min - max; years) | TR (s) | Volumes | Threshold with best power-law fit | $\alpha$ | $\tau$ | $\gamma$ | $\frac{\sigma - 1}{\alpha - 1}$ | $\sigma$ |
| ---- | ----- | ------ | ------ | ------- | ----- | ----- | ----- | ----- | --- | --- |
| @tagliazucchiCriticalityLargeScaleBrain2012 | 10 | 49 $\pm$ 12  | 2.5 | 240 | 1 SD | No exponent calculated | No power law | NA | NA | NA |
| @xuAvalancheCriticalityIndividuals2022 | 295 | 22 - 36 | 0.72 | 1200 | 1.4 SD | 1.56 | 1.84 | 1.59 | 1.51 | 0.9097 |
| @xinGeneticContributionsBrain2025 | MZ 250, DZ 142, NT 437 | MZ 29 $\pm$ 3, DZ 29 $\pm$ 4, NT 28 $\pm$ 4 | 0.72 | 1200 | $\pm$ 1.75 SD | MZ 1.60, DZ 1.58, NT 1.58 | MZ 1.96, DZ 1.93, NT 1.92 | MZ 1.60, DZ 1.58, NT 1.62 | MZ 1.60, DZ 1.59, NT 1.58 | MZ 1.00, DZ 1.02, NT 1.02 |
| @xinElectroconvulsiveTherapyModulates2022 | MDD 42, HC 42 | 20 - 60 | 1 | 217 | 2.2 SD | 1.5 | NA | NA | NA | 1 |
: **A summary of fMRI avalanche studies.** Includes stats and relevant findings. For sample size and age, commas separate cohorts. @bocaccioModelbasedMachineLearning2024 was excluded as it did not include any measure of duration. Power-law fit equations and ideal exponents are obtained from cited papers. Exponent $\alpha$ for power-law fit for relationship between distribution of sizes ($P(S)$) and size ($S$); $\alpha$ = 1.5 indicates criticality. Exponent $\tau$ for power-law fit for relationship between distribution of durations ($P(T)$) and duration ($T$); $\tau$ = 2 indicates criticality. Exponent $\gamma$ of power-law fit for relationship between average size ($<S>$) for a given duration ($T$). $\gamma$ should equal $\frac{\sigma - 1}{\alpha - 1}$ for critical systems. $\sigma$ is the branching ratio, where $\sigma = \frac{1}{N}\sum_{i=1}^{N}\frac{n^{*}_{i}}{n_{i}}$, $n_i$ is the number of ancestors in a preceding time bin, $n_i^*$ is the number of descendants in the next time bin, and $N$ is the total number of active time bins; $\sigma = 1$ indicates criticality. MZ = monozygotic twins; DZ = dizogotic twins; NT = non-twin {#tbl-avalanche}

# Hurst exponent

Let us move on to fractality and the Hurst exponent. A fractal structure, either spatial or temporal, is composed of smaller parts that exhibit the same pattern at every scale [@kochCourbeContinueSans1904;@kochMethodeGeometriqueElementaire1906; @mandelbrotHowLongCoast1967]. Classic spatial examples of fractals in nature are coastlines [@mandelbrotHowLongCoast1967], circulatory system [@jayalalithaFractalModelBlood2008], and brain anatomy [@ansellUnveilingUniversalAspects2024], where at each level of magnification these structures look the same; i.e. *self-similarity* (See @fig-sierpinskitriangle for an idealized mathematical example). For a dynamic process, or in the temporal domain, this is known as *scale invariance*, meaning that both rapid and slow processes follow the same pattern [@rileyTutorialIntroductionAdaptive2012]. Scale invariance exhibits a gradual decay in its autocorrelation function, known as scale-free long-range correlations (LTRCs). LTRCs are foundational to the measurement of temporal fractality as they indicate that the system maintains a long-lasting, multiscale memory of its past states, which play a role in shaping its present behaviour.
...
The Hurst exponent (HE) is a statistical measure used to quantify persistence in time-series data and long-range temporal correlations (LRTC). First introduced by Harold Hurst in 1951 for hydrological applications [@hurstLongTermStorageCapacity1951], it was Benoit Mandelbrot who helped popularize its use for studying complex shapes and forms of nature [@mandelbrotClasseProcessusStochastiques1965; @mandelbrotHowLongCoast1967]. Since then it has been applied across various fields, including finance, geophysics, ecology, and neuroscience, to characterize complex systems with intricate dynamics [@molzFractionalBrownianMotion1997; @korvinFractalModelsEarth1992; @parkSelfSimilarNetworkTraffic2000; @gravesBriefHistoryLong2017]. HE and FD for time-series can be directly converted, as $HE=2–FD$. This means that FD and HE are inversely correlated, and that HE is a real number between 0 and 1 (non-inclusive; although it can be extended to > 1; see [@ekePhysiologicalTimeSeries2000; @hartmannRealtimeFractalSignal2013]). For the rest of this paper, we will be discussing HE.

*Thus, the basic premise behind fractal time-series analysis is: beneath the seemingly chaotic and unpredictable variations in the signal, there lies a stable underlying mechanism which can be effectively described using only a few parameters (i.e. HE or FD).*
...
<!-- TODO: could be better fleshed out to make it flow -->
...
## A4. Comparisons of temporal complexity and amplitude of low-frequency fluctuations
